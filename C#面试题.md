...
* Clear()
* Contains(key)
1. 遍历哈希表用DictionaryEntry Object
...
## C#特点
1. 简单易学
2. 面向对象，支持封装、继承和多态，允许开发者使用类、接口、委托等概念组织和管理代码
3. 类型安全，强类型语言，对数据类型严格检查和限制，减少类型错误和潜在的运行时异常
4. 内存管理，自动垃圾回收机制，无需手动分配和释放内存，自动检测不再